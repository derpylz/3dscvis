...
var SCVis = /** @class */ (function () {
    ...
    SCVis.prototype._updateTimeSeriesCells = function () {
        ...
        if (this._timeSeriesIndex > Math.max.apply(Math, this._timeSeriesVariable)) {
            this._timeSeriesIndex = 0;
            var indexBefore = Math.max.apply(Math, this._timeSeriesVariable);
    ...
    SCVis.prototype._updateTimeSeriesLabels = function () {
        for (var labelId in this._labels) {
            if (this._labels.hasOwnProperty(labelId)) {
                var label = this._labels[labelId];
                if (label.timeLinked) {
    ...
    SCVis.prototype._showAllLabels = function () {
        for (var labelId in this._labels) {
            if (this._labels.hasOwnProperty(labelId)) {
                var label = this._labels[labelId];
                label.mesh.visibility = 1;
     ...
     */
    SCVis.prototype.enableTimeSeries = function (timeSeriesVariable) {
     ...
     * @param timeSeriesVariable Array of same length as number of cells indicating time point to show cell
     */
    SCVis.prototype.setTimeVariable = function (timeSeriesVariable) {
        this._timeSeriesVariable = timeSeriesVariable;
        return this;
    };
    /**
     ...
     */
    SCVis.prototype.timeLinkLabel = function (labelId, linking) {
        var label = this._labels[labelId];
        label.timeLinked = true;
        label.linkedTo = [];
        linking = linking.replace(/\s/g, '');
        var linkArray = linking.split(",");
        for (var idx = 0; idx < linkArray.length; idx++) {
            var link = linkArray[idx];
            if (link.indexOf("-") > -1) {
                var lA = link.split("-");
                for (var i = parseInt(lA[0]); i <= parseInt(lA[1]); i++) {
